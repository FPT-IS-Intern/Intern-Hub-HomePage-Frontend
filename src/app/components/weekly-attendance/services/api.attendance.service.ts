        ...
        return this.http.get<ApiResponse<any>>(`${this.API_BASE_URL}/status`, { params })
            .pipe(
                map(res => {
                    ...
                    if (data.isCheckInValid !== undefined) summary.checkIn.isCheckTimeValid = data.isCheckInValid;
                    if (data.isCheckOutValid !== undefined) summary.checkOut.isCheckTimeValid = data.isCheckOutValid;
